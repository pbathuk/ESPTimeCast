...
void setupWebServer() {
  ...
  server.on("/save", HTTP_POST, 
    ...
    [](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total) {
      ...
      // If the webpage didn't send it, getJsonValue returns the current global value (preserving it)
      // or the default if we force it.
      ...
      if (doc["language"]) strlcpy(language, doc["language"], sizeof(language));
      if (doc["timeZone"]) strlcpy(timeZone, doc["timeZone"], sizeof(timeZone));
      Serial.print("pre-timeZone set to: ");
      Serial.println(timeZone);
      if (doc["weatherUnits"]) strlcpy(weatherUnits, doc["weatherUnits"], sizeof(weatherUnits));
      if (doc["customMessage"]) strlcpy(customMessage, doc["customMessage"], sizeof(customMessage));

      if (doc["ntpServer1"]) strlcpy(ntpServer1, doc["ntpServer1"], sizeof(ntpServer1));
      if (doc["ntpServer2"]) strlcpy(ntpServer2, doc["ntpServer2"], sizeof(ntpServer2));
      ...
      if (doc["haTempSensor"]) strlcpy(haTempSensor, doc["haTempSensor"], sizeof(haTempSensor));
      if (doc["haHumiditySensor"]) strlcpy(haHumiditySensor, doc["haHumiditySensor"], sizeof(haHumiditySensor));
      
      if (doc["openWeatherCity"]) strlcpy(openWeatherCity, doc["openWeatherCity"], sizeof(openWeatherCity));
      if (doc["openWeatherCountry"]) strlcpy(openWeatherCountry, doc["openWeatherCountry"], sizeof(openWeatherCountry));
      

      // Security Checks (Password & API Keys)
      String newssid = getJsonValue(doc, "ssid", String(""));
      if (newssid != "********" && newssid.length() > 0) strlcpy(ssid, newssid.c_str(), sizeof(ssid));

      String newPass = getJsonValue(doc, "password", String(""));
      if (newPass != "********" && newPass.length() > 0) strlcpy(password, newPass.c_str(), sizeof(password));

      String newOwKey = getJsonValue(doc, "openWeatherApiKey", String(""));
      if (newOwKey != "********************************" && newOwKey.length() > 0) strlcpy(openWeatherApiKey, newOwKey.c_str(), sizeof(openWeatherApiKey));

      String newHaKey = getJsonValue(doc, "homeAssistantApiKey", String(""));
      if (newHaKey != "********************************" && newHaKey.length() > 0) strlcpy(homeAssistantApiKey, newHaKey.c_str(), sizeof(homeAssistantApiKey));
      ...
      // We must clear and add EVERYTHING back, otherwise missing items get deleted from the file.
      doc.clear();
      ...
      if (verifyErr) {
        request->send(500, "application/json", "{\"error\":\"Config corrupted\"}");
        return;
      }
