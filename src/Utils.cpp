...
textEffect_t getEffectiveScrollDirection(textEffect_t desiredDirection,
                                         bool isFlipped) {
  if (isFlipped) {
    // If the display is horizontally flipped, reverse the horizontal scroll
    // direction
...
void advanceDisplayMode() {
  ...
  if (displayMode == 0) { // Clock
    if (showDate) {
      ...
      Serial.println(
          F("[DISPLAY] Switching to display mode: DATE (from Clock)"));
               ...
               ((!useHomeAssistant && strlen(openWeatherApiKey) == 32 &&
                ...
                (useHomeAssistant && strlen(homeAssistantApiKey) > 32 &&
                 strlen(haTempSensor) > 0) // Or HomeAssistant setup
                )) {
      displayMode = 1;
      Serial.println(
          F("[DISPLAY] Switching to display mode: WEATHER (from Clock)"));
    } else if (countdownEnabled && !countdownFinished && ntpSyncSuccessful &&
               countdownTargetTimestamp > 0 &&
               countdownTargetTimestamp > time(nullptr)) {
      displayMode = 3;
      Serial.println(F("[DISPLAY] Switching to display mode: COUNTDOWN (from "
                       "Clock, weather skipped)"));
    } else {
      displayMode = 0;
    ...
    if (weatherAvailable &&
      ...
      displayMode = 1;
      Serial.println(
          F("[DISPLAY] Switching to display mode: WEATHER (from Date)"));
    } else if (countdownEnabled && !countdownFinished && ntpSyncSuccessful &&
               countdownTargetTimestamp > 0 &&
               countdownTargetTimestamp > time(nullptr)) {
      displayMode = 3;
      Serial.println(F("[DISPLAY] Switching to display mode: COUNTDOWN (from "
                       "Date, weather skipped)"));
    } else {
      displayMode = 0;
      Serial.println(
          F("[DISPLAY] Switching to display mode: CLOCK (from Date)"));
    }
  } else if (displayMode == 1) { // Weather
    if (showWeatherDescription && weatherAvailable &&
        weatherDescription.length() > 0) {
      displayMode = 2;
      Serial.println(
          F("[DISPLAY] Switching to display mode: DESCRIPTION (from Weather)"));
    } else if (countdownEnabled && !countdownFinished && ntpSyncSuccessful &&
               countdownTargetTimestamp > 0 &&
               countdownTargetTimestamp > time(nullptr)) {
      displayMode = 3;
      Serial.println(
          F("[DISPLAY] Switching to display mode: COUNTDOWN (from Weather)"));
    } else {
      displayMode = 0;
      Serial.println(
          F("[DISPLAY] Switching to display mode: CLOCK (from Weather)"));
    }
    ...
    if (countdownEnabled && !countdownFinished && ntpSyncSuccessful &&
      ...
      displayMode = 3;
      Serial.println(F(
          "[DISPLAY] Switching to display mode: COUNTDOWN (from Description)"));
    } else {
      displayMode = 0;
      Serial.println(
          F("[DISPLAY] Switching to display mode: CLOCK (from Description)"));
    }
  } else if (displayMode == 3) { // Countdown --> Clock
    displayMode = 0;
    Serial.println(
        F("[DISPLAY] Switching to display mode: CLOCK (from Countdown)"));
  } else if (displayMode == 4) { // Blankscreen -> Custom Message
    displayMode = 6;
    Serial.println(F("[DISPLAY] Switching to display mode: CUSTOM MESSAGE "
                     "(from BlankScreen)"));
  } else if (displayMode == 6) { // Custom Message -> Clock
    displayMode = 0;
    Serial.println(
        F("[DISPLAY] Switching to display mode: CLOCK (from Custom Message)"));
  }
  ...
  if ((displayMode == 0) && strlen(customMessage) > 0 && oldMode != 6) {
    displayMode = 6;
    Serial.println(
        F("[DISPLAY] Custom Message display before returning to CLOCK"));
  }
  lastSwitch = millis();
}
...
void advanceDisplayModeSafe() {
  ...
  do {
    ...
    if (displayMode == 0)
      ...
      valid = true;
    else if (displayMode == 2 && showWeatherDescription && weatherAvailable &&
             weatherDescription.length() > 0)
      valid = true;
    else if (displayMode == 3 && countdownEnabled && !countdownFinished &&
             ntpSyncSuccessful)
      valid = true;
    else if (displayMode == 6 && strlen(customMessage) > 0)
      valid = true;

    // If we've looped back to where we started, break to avoid infinite loop
    if (displayMode == startMode)
      break;
...
String getHAJSON(String entityID, JsonDocument &doc) {
  ...
  http.addHeader("content-type",
                 "application/json"); // Ensures that the content returned is in
                                      // json format
  http.addHeader("User-Agent",
                 hostName); // Sets the User-Agent to Current value of hostName
  http.addHeader("Authorization",
                 String("Bearer ") +
                     homeAssistantApiKey); // Add the API key to the header
  Serial.println(F("[HOME ASSISTANT] Sending GET request..."));
  int httpCode = http.GET(); // Send the GET request
  if (httpCode != HTTP_CODE_OK) {
    Serial.printf(
        "[HOME ASSISTANT] HTTP GET failed, error code: %d, reason: %s\n",
        httpCode, http.errorToString(httpCode).c_str());
    Serial.println(
        F("[HOME ASSISTANT] Home Assistant requested, but not configured"));
    Serial.println(
        F("[HOME ASSISTANT] Setting Use Home Assistant flag to off"));
...
String buildWeatherURL() {
  ...
  base += "&lang=en"; // Force English for consistent description parsing
...
const char *getSafePassword() {
  if (strlen(password) ==
      0) { // No password set yet — return empty string for fresh install
    return "";
...
void ensureHtmlFileExists() {
  ...
  if (LittleFS.exists("/index.html")) {
    ...
    if (!f) {
      Serial.println(F(
          "[FS] ERROR: /index.html exists but failed to open! Will rewrite."));
    } else {
      size_t actualSize = f.size();
      f.close();

      if (actualSize == expectedSize) {
        Serial.printf("[FS] /index.html found (size OK: %u bytes). Using file "
                      "system version.\n",
                      actualSize);
        return; // STOP HERE — file is good
      }

      Serial.printf("[FS] /index.html size mismatch! Expected %u bytes, found "
                    "%u. Rewriting...\n",
                    expectedSize, actualSize);
    }
  } else {
    Serial.println(F(
        "[FS] /index.html NOT found. Writing embedded content to LittleFS..."));
  }
  ...
  if (bytesWritten == htmlLength) {
    ...
    Serial.printf("[FS] WARNING: Only wrote %u of %u bytes to /index.html "
                  "(might be incomplete).\n",
